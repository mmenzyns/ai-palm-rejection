...
chosen_features = ['msdxTF', 'mmeanx', 'std']

features = pd.concat((illegal_features, legal_features))
X_train = features.drop('target', axis=1) if len(chosen_features) == 0 else features[chosen_features]
...
X_test = features.drop('target', axis=1) if len(chosen_features) == 0 else features[chosen_features]
...
negatives = np.sum(y_pred)
positives = y_pred.size - negatives

# print("real negatives ", np.sum(y_test))
# print("real positives ", y_test.size - np.sum(y_test))
